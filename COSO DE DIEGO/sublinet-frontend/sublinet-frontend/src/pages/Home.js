...
const apiPost = async (path, body) => {
  ...
  // Puede venir vacío (201 sin body)
...
const apiPut = async (path, body) => {
  ...
  // Para 204 NoContent o respuestas sin cuerpo
  return text ? JSON.parse(text) : null;
};
...
// --- CONTENEDOR PARA TARJETAS DE PRODUCTOS (VENTAS) ---
...
// Imagenes demo para algunas camisas (si el backend aún no tiene imagen)
const PRODUCT_IMAGES = {
  'Camisa Azul': 'https://png.pngtree.com/png-vector/20240124/ourmid/pngtree-isolated-blue-t-shirt-png-image_11479977.png',
  'Camisa Blanca': 'https://png.pngtree.com/png-vector/20240815/ourmid/pngtree-classic-white-t-shirt-perfect-for-custom-printing-with-a-blank-png-image_13485404.png',
  'Camisa Negra': 'https://png.pngtree.com/png-vector/20240401/ourmid/pngtree-black-t-shirt-mockup-transparent-background-png-image_12256281.png',
  'Camisa Roja': 'https://static.vecteezy.com/system/resources/thumbnails/037/752/398/small/ai-generated-photo-of-clean-red-t-shirt-without-background-template-for-mockup-png.png',
};
const DEFAULT_PRODUCT_IMAGE = 'https://via.placeholder.com/300x150.png?text=Producto';
...
const Home = ({ onLogout }) => {
    ...
    const loadData = async () => {
      try {
        ...
        await Promise.allSettled([
          ...
          (async () => {
            try {
              const b = await apiGet('/Budgets/latest');
              setBudget(b || null);
            } catch {
              setBudget(null);
            }
          ...
          (async () => {
            try {
              const inv = await apiGet('/Invoices/pending/latest');
              setPendingInvoice(inv || null);
            } catch {
  ...
  // ==== DATOS DERIVADOS A PARTIR DEL BACKEND =====
  ...
  // ===== CRUD PRODUCTOS (máx 4) =====
  const handleAddProduct = async () => {
    if (products.length >= 4) {
      alert('Solo se permiten 4 productos en el sistema.');
      return;
    }
    ...
    const priceStr = prompt('Precio (ej: 20.5):', '0');
  ...
  const handleEditProduct = async (item) => {
    ...
    const priceStr = prompt('Nuevo precio:', item.price);
    const price = parseFloat(priceStr || item.price);

    const stockStr = prompt('Nuevo stock:', item.quantity);
    const stock = parseInt(stockStr || item.quantity, 10);
  ...
  const handleDeleteProduct = async (item) => {
    ...
    try {
      await apiDelete(`/Products/${item.id}`);
      await reloadProducts();
    } catch (err) {
      console.error(err);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <GridContainer>
        <Card>
          ...
          <StatNumber>
            {inventoryData.reduce((acc, item) => acc + (item.quantity || 0), 0)}
          </StatNumber>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <StatNumber color={ACCENT_COLOR}>0</StatNumber>
        </Card>
      </GridContainer>
          ...
          <div>
            ...
            <ProgressBar>
              <ProgressFill
                percentage={
                  budgetData.total > 0
                    ? (budgetData.used / budgetData.total) * 100
                    : 0
                }
              />
            </ProgressBar>
  ...
  const InventoryContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <FlexBetween>
        ...
        <Button onClick={handleAddProduct} disabled={products.length >= 4}>
          {products.length >= 4 ? 'Máx 4 productos' : 'Agregar Producto'}
        </Button>
      </FlexBetween>
        ...
        <tbody>
          {inventoryData.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell>
                <ActionButton color={ACCENT_COLOR} onClick={() => handleEditProduct(item)}>
                  Editar
                </ActionButton>
                <ActionButton color={DANGER_RED} onClick={() => handleDeleteProduct(item)}>
                  Eliminar
                </ActionButton>
              </TableCell>
            </TableRow>
  ...
  const BudgetContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
        ...
        {/* Esto lo podés ajustar después; por ahora es demo */}
        <Card>
  ...
  // --- SALES CONTENT USANDO PRODUCTS DEL BACKEND (máx 4) ---
  const SalesContent = ({ products }) => {
    const [sales, setSales] = useState([]);

    useEffect(() => {
      const initial = products.slice(0, 4).map(p => ({
        id: p.id,
        name: p.name,
        image: PRODUCT_IMAGES[p.name] || DEFAULT_PRODUCT_IMAGE,
        sold: 0,
      }));
      setSales(initial);
    }, [products]);
    ...
    const handleRegisterSale = (id) => {
      ...
      alert(`Venta registrada (demo): ${product.sold} unidades de ${product.name}`);
      // Aquí después se puede llamar al backend para crear Invoice / detalle.
    };
  ...
  const BillingContent = () => {
    const invoice = pendingInvoice && pendingInvoice.id
      ? {
          id: `#${pendingInvoice.invoiceNumber || pendingInvoice.id}`,
          client: pendingInvoice.customer?.name || 'Cliente',
          date: new Date(pendingInvoice.date).toLocaleDateString('es-NI'),
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <BillingGrid>
          <Card style={{ background: `linear-gradient(135deg, ${PRIMARY_DARK_BLUE}, ${HOVER_DARK_BLUE})`, color: WHITE }}>
            <StatTitle style={{ color: WHITE, borderBottom: `1px solid ${ACCENT_COLOR}`, paddingBottom: '0.5rem' }}>
              Pagar Factura Pendiente
            </StatTitle>

            {!invoice ? (
              <p style={{ marginTop: '1.5rem', color: ACCENT_COLOR }}>
                No hay factura pendiente o no se pudo cargar desde el backend.
              </p>
            ) : (
              <div style={{ marginTop: '1.5rem' }}>
                <FlexBetween style={{ marginBottom: '1rem' }}>
                  <span style={{ fontSize: '1rem', color: ACCENT_COLOR }}>Factura ID:</span>
                  <span style={{ fontSize: '1.125rem', fontWeight: '600' }}>{invoice.id}</span>
                </FlexBetween>
                <FlexBetween style={{ marginBottom: '1.5rem' }}>
                  <span style={{ fontSize: '1rem', color: ACCENT_COLOR }}>Cliente:</span>
                  <span style={{ fontSize: '1.125rem', fontWeight: '600' }}>{invoice.client}</span>
                </FlexBetween>
        ...
        {/* Historial de ejemplo estático por ahora */}
        <div style={{marginTop: '1.5rem'}}>
          ...
          <Table>
            ...
            <tbody>
              ...
              {invoice && (
                <TableRow>
                  <TableCell style={{ fontWeight: '500', color: HOVER_DARK_BLUE }}>{invoice.id}</TableCell>
                  <TableCell color={TEXT_SECONDARY}>{invoice.client}</TableCell>
                  <TableCell color={TEXT_SECONDARY}>{invoice.date}</TableCell>
                  <TableCell color={TEXT_SECONDARY}>
                    ${invoice.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </TableCell>
                  <TableCell>
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case 'sales':
        return <SalesContent products={products} />;
      case 'billing':
  ...
  return (
    <HomeContainer>
      ...
      <Body toggleSidebar={toggleSidebar}>
        {loading && <p style={{ color: 'white' }}>Cargando datos del sistema...</p>}
        {error && (
